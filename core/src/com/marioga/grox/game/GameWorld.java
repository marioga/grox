...
import com.marioga.grox.game.objects.Player.Level;
...
public class GameWorld {
	...
	private void init() {
		...
		dimensionTile = (float)(3*Constants.VIEWPORT_WIDTH / 4)/(float) gameBoardWidth;
		
		//Create the board
		gameBoard = new GameBoard(gameBoardWidth, gameBoardHeight);
		gameBoard.blockDimension = dimensionTile;
		
		//Create the players
		players = new Array<Player>();
		players.add(new Player(Constants.FIRST_PLAYER_LEVEL, true, new Color(0.7f,0.1f,0.1f,1.0f)));
		players.add(new Player(Constants.SECOND_PLAYER_LEVEL, false, new Color(0.1f,0.1f,0.7f,1.0f)));
		
		//Create the blocks
		Array<Block> firstPlayerBlocks = new Array<Block>();
		Array<Block> secondPlayerBlocks = new Array<Block>();
		for (BlockType bt : BlockType.values()) {
			firstPlayerBlocks.add(new Block(bt, 0, dimensionTile, getFirstPlayer()));
			secondPlayerBlocks.add(new Block(bt, 0, dimensionTile, getSecondPlayer()));
		}
	...
	public void updateWorldDimensions(float viewportHeight) {
		if (getFirstPlayer()!=null) {
			for (Block block : getFirstPlayer().getBlocks()) {
				block.setInitialScaleFactor((viewportHeight - gameBoard.getHeight()*dimensionTile)/18/dimensionTile);
				block.updateInitialPosition(0, gameBoard.centerPosition.y - gameBoard.getHeight()*dimensionTile/2 
						- 2.5f*dimensionTile*block.getInitialScaleFactor());
		...
		if (getSecondPlayer()!=null) {
			for (Block block : getSecondPlayer().getBlocks()) {
				block.setInitialScaleFactor((viewportHeight - gameBoard.getHeight()*dimensionTile)/18/dimensionTile);
				block.updateInitialPosition(0, gameBoard.centerPosition.y + gameBoard.getHeight()*dimensionTile/2 
						+ 6.5f*dimensionTile*block.getInitialScaleFactor());
				if (block.getBlockState() == BlockState.IDLE) {
					block.setCurrentScaleFactor(block.getInitialScaleFactor());
					block.getCenterBlockPosition().set(block.getInitialPosition());
				}
	...
	public static boolean isValidMove(int[][] boardMatrix, int[][] blockMatrix, int row, int column) {
		for (int i = -1; i<=1; i++) {
			for (int j = -1; j<=1; j++) {
				if ((blockMatrix[i+1][j+1]==1)&&(boardMatrix[row+i][column+j]!=0))
					return false;
			}
		}
		return true;
	...
	public boolean isValidMove(int row, int column, Block block) {
		return isValidMove(gameBoard.getGameBoardModel(), block.getMatrixRepresentation(), row, column);
	}
	...
	public void clearPieceFromBoard(int row, int column, Block block) {
		for (int i = -1; i<=1; i++) {
			for (int j = -1; j<=1; j++) {
				if ((gameBoard.getGameBoardModel()[row+i][column+j]==1)&&(block.getMatrixRepresentation()[i+1][j+1]==1))
	...
	public boolean rotateInBoard(Block block) {
		...
		while (!isValidMove(getGameBoard().getGameBoardModel(), block.getMatrixRepresentation(), 
				row, column)) {
			block.rotateBlockTo(currentRotation++); 
		}
			...
			for (int rot = 0; rot<block.getNumberOfRotatedForms(); rot++) {
				for (int row=1; row<=getGameBoard().getHeight(); row++) {
					for (int column=1; column<=getGameBoard().getWidth(); column++) {
						if (isValidMove(getGameBoard().getGameBoardModel(), block.getRotatedMatrixRepresentation(rot), 
								row, column)) {
							return false;
						}
					}
