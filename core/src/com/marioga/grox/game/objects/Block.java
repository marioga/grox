...
public class Block {
	...
	public static enum BlockType {
		...
		public int[][] getMatrixRepresentation(int rotation) {
			...
			for (int i = rotation*10; i<rotation*10+9; i++) {
				...
				matrix[row][column] = Character.getNumericValue(representation.charAt(i));
			}
			return matrix;
		}
	...
	private int currentRotation; //Integer in [0,3] giving rotation divided by 90
	...
	private Vector2 initialPosition; //Position of center of center block
	
	public Vector2 getInitialPosition() {
		return initialPosition;
	}

	private Vector2 centerBlockPosition; //Position of center of center block
	...
	public void rotateBlockTo(int rotation) {
		...
		matrixRepresentation = blockType.getMatrixRepresentation(currentRotation);
	}
	
	public void moveToPositionInBoard(int row, int column, Vector2 boardCenterPosition, int boardWidth, int boardHeight) {
		centerBlockPosition.x = boardCenterPosition.x + (column - (float)(boardWidth+1)/2)*blockDimension;
		centerBlockPosition.y = boardCenterPosition.y - (row - (float)(boardHeight+1)/2)*blockDimension;
	}
		...
		for (int i = 0; i<3; i++) {
			for (int j = 0; j<3; j++) {
				if (matrixRepresentation[i][j]==1) {
					batch.draw(region, centerBlockPosition.x+(j-1)*dimen-dimen/2, 
							centerBlockPosition.y-(i-1)*dimen-dimen/2,
							dimen, dimen,
							0,0,region.getWidth(),region.getHeight(),
							false, false);
				}
			}
		}
	...
	public int[][] getRotatedMatrixRepresentation(int rotation) {
		...
		return blockType.getMatrixRepresentation((numForms + rotation) % numForms);
	}
}
