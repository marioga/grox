...
public class PlayerAI {
	public static GroxPlay computeNextPlay(GameBoard board, Player player, Player opponent) {
		...
		case BEGINNER:
			return computeNextPlayBeginner(board.getGameBoardModel(), copyPlayerBlocks, copyOpponentBlocks);
		case INTERMEDIATE:
			return computeNextPlayIntermediate(board.getGameBoardModel(), copyPlayerBlocks, copyOpponentBlocks);
		case ADVANCED:
			return computeNextPlayAdvanced(board.getGameBoardModel(), copyPlayerBlocks, copyOpponentBlocks);
		case EXPERT:
			return computeNextPlayExpert(board.getGameBoardModel(), copyPlayerBlocks, copyOpponentBlocks);
		default:
			return null;
		}
	}

	private static GroxPlay computeNextPlayBeginner(int[][] boardModel, Array<Block> playerBlocks, 
			Array<Block> opponentBlocks) {
			...
			for (int rot = 0; rot<block.getNumberOfRotatedForms(); rot++) {
				for (int row=1; row<boardModel.length-1; row++) {
					for (int column=1; column<boardModel[0].length-1; column++) {
						if (GameWorld.isValidMove(boardModel, block.getRotatedMatrixRepresentation(rot),
								row, column)) {
	...
	private static GroxPlay computeNextPlayIntermediate(int[][] boardModel, Array<Block> playerBlocks, 
			Array<Block> opponentBlocks) {
		...
		if (getPlayNumber(boardModel)==1) {
			return computeNextPlayBeginner(boardModel, playerBlocks, opponentBlocks);
		}
			...
			for (int rot = 0; rot<block.getNumberOfRotatedForms(); rot++) {
				for (int row=1; row<boardModel.length-1; row++) {
					for (int column=1; column<boardModel[0].length-1; column++) {
						if (GameWorld.isValidMove(boardModel, block.getRotatedMatrixRepresentation(rot),
								row, column)) {
							GroxPlay play = new GroxPlay(row, column, block, rot);
							int oppPlays = computePlaysAfter(play, boardModel, opponentBlocks, true);
							if (oppPlays==0) {
								return play;
							}
							int myPlays = computePlaysAfter(play, boardModel, playerBlocks, false);
							if ((bestPlay==null)||(myPlays-oppPlays>maxPlays)) {
								bestPlay = play;
	...
	private static int computePlaysAfter(GroxPlay play, int[][] boardModel, Array<Block> blocks, boolean opponent) {
		...
		for (Block block : tempArray) {
			for (int rot = 0; rot<block.getNumberOfRotatedForms(); rot++) {
				for (int row=1; row<boardModel.length-1; row++) {
					for (int column=1; column<boardModel[0].length-1; column++) {
						if (GameWorld.isValidMove(boardCopy, block.getRotatedMatrixRepresentation(rot),
								row, column)) {
							counter++;
						}
					}
	...
	private static GroxPlay computeNextPlayAdvanced(int[][] boardModel, Array<Block> playerBlocks, 
			Array<Block> opponentBlocks) {
				...
				for (int row=1; row<boardModel.length-1; row++) {
					for (int column=1; column<boardModel[0].length-1; column++) {
						if (GameWorld.isValidMove(boardModel, block.getRotatedMatrixRepresentation(rot),
								row, column)) {
							GroxPlay play = new GroxPlay(row, column, block, rot);
							int minPlaysAfter = computeMinPlaysAfter(play, boardModel, playerBlocks, opponentBlocks);
							if ((minPlaysAfter==-1)||(minPlaysAfter>=20)) { //-1 will mean no more plays left for opponent, 20 is a reasonable amount of plays
								return play;
							}
							if ((bestPlay==null)||(minPlaysAfter>maxMinPlaysAfter)) {
								bestPlay = play;
				...
				for (int row=1; row<boardModel.length-1; row++) {
					for (int column=1; column<boardModel[0].length-1; column++) {
						if (GameWorld.isValidMove(boardCopy, block.getRotatedMatrixRepresentation(rot),
								row, column)) {
							counter++;
							GroxPlay opponentPlay = new GroxPlay(row, column, block, rot);
							int myPlaysAfterOpponent = computePlaysAfter(opponentPlay, boardCopy, copyPlayerArray, false);
	...
	private static int[][] getCopyOfBoardAfterPlay(int[][] boardModel, GroxPlay play) {
		...
		for (int i = -1; i <= 1; i++) {
			for (int j = -1; j <= 1; j++) {
				if (play.getBlock().getRotatedMatrixRepresentation(play.getBlockRotation())[i+1][j+1]==1) {
				...
				for (int row=1; row<boardModel.length-1; row++) {
					for (int column=1; column<boardModel[0].length-1; column++) {
						if (GameWorld.isValidMove(boardModel, block.getRotatedMatrixRepresentation(rot),
								row, column)) {
							GroxPlay play = new GroxPlay(row, column, block, rot);
							int losses = computeNumberOfLosses(play, boardModel, playerBlocks, opponentBlocks);
							if (losses==0) {
				...
				for (int row=1; row<boardCopy.length-1; row++) {
					for (int column=1; column<boardCopy[0].length-1; column++) {
						if (GameWorld.isValidMove(boardCopy, block.getRotatedMatrixRepresentation(rot),
								row, column)) {
							GroxPlay opponentPlay = new GroxPlay(row, column, block, rot);
							if (computeNumberOfLosses(opponentPlay, boardCopy, opponentBlocks, copyPlayerArray)==0) {
								numLosses++;
							}
						}
					}
